const game = {
    ...
    if (level < "4") {
      ...
      this.interval = setInterval(() => {
        this.framesCounter > 5000
          ...
          : this.framesCounter++;
        //   this.framesCounter++;
        //   this.framesCounter % this.numberEnemies === 0
        //     ? this.createEnemies()
        //     : null;
        this.cleanWindow();
        this.drawAll();
  ...
  //   randomValuesEnemies() {
  //     this.enemy.pos.x = Math.floor(Math.random() * this.gameSize.w);
  //     //this.enemy.pos.y;
  //     this.enemy.vel.x = Math.floor(Math.random() * 100);
  //     this.enemy.vel.y = Math.floor(Math.random() * (3 - 1) + 1);
  //   },
  ...
  enemiesShoot() {
    ...
    if (this.framesCounter % this.shootsVelocity === 0) {
      // this.grids.enemies[0].bullets[0].makeSound();
  ...
  gameOver() {
    ...
    finalScore.innerText = `YOUR SCORE IS ${
      document.querySelector("#score span").innerText
    }`;
